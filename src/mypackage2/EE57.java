...
import mypackage.AdditionQuestion;

/**
 * This program administers a ten-question addition quiz to the user. The
 * numbers for the problem are chosen at random. The numbers and the answers are
 * one or two digits. After asking the user the ten questions, the computer
 * grades the quiz, telling the user the correct answer for any problem they got
 * wrong.
 */
public class EE57 {
	...
	private static int[] userAnswers; // The user's answers to the ten
										// questions.
	...
	private static void administerQuiz() {
		...
		for (int i = 0; i < 10; i++) {
			int questionNum = i + 1;
			System.out.printf("Question %2d:  %s ", questionNum,
	 ...
	 * for the quiz. For each question, the user is told whether they got it
	 * right.
	 */
	private static void gradeQuiz() {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (userAnswers[i] == questions[i].getCorrectAnswer()) {
				...
				System.out.println("You said " + userAnswers[i]
						+ ", which is INCORRECT.");
			}
		}
